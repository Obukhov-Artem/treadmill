...
class TreadmillControl(QMainWindow):
    def __init__(self):
        ...
        self.ard_trackers = b'LHR-9D5EB008'
        self.arduino = None
        if self.arduino:
            try:
                self.ard_connect()
            ...
            for device in v.devices:
                ...
                if position_device:
                    ...
                    if v.devices[device].device_class == 'HMD':
                        ...
                        if v.devices[device].get_serial() == SERIAL or v.devices[device].get_serial() == self.ard_trackers or\
                                v.devices[device].get_serial() == SERIAL.encode():
                            print("OK")
