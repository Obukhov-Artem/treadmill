    ...
    def recording_while(self):
        ...
        while self.RecordingWhile:
            ...
            if len(data) == 0:
                ...
                self.data_last = data_current
            else:
                bufer = [self.data_last[i]-data_current[i] for i in range(len(data_current)-1)]
                bufer.append(self.current_speed)
                data.append(bufer)
                data.append(datetime.utcnow().strftime('%H:%M:%S.%f')[:-3])
                self.data_last = data_current
            n += 1
            if n >= 100000:
