...
class TreadmillControl(QMainWindow):
    ...
    def get_speed(self,z):
        ...
        if self.z_napr > 0:
            ...
            if int(abs(z * acceleration_factor)) > 0:
                self.arduino.write(
                    bytes(str(int(abs(arr[-1] * drag_coefficient) * 1.1)) + '.', 'utf-8'))  # Изначально cof = 200
                self.Display.display(int(abs(arr[-1] * drag_coefficient) * 1.1))
            elif int(abs(arr[-1] * acceleration_factor)) <= 0:
                self.arduino.write(
                    bytes(str(int(abs(arr[-1] * acceleration_factor))) + '.', 'utf-8'))  # Изначально factor = 200
